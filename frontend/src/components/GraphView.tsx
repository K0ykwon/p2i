...
function GraphCanvas({ir,ops,select,selected,focusId}:{ir:IR;ops:Operation[];select:(s:Selection)=>void;selected:Selection;focusId?:string}) {
  ...
  const {positions,edges}=useMemo(()=>{
    ...
    const levels=new Map<string,number>(); const rows=new Map<number,number>();
    const incoming=new Map<string,string[]>();
    for(const e of edges) incoming.set(e.target_id,[...(incoming.get(e.target_id)??[]),e.source_id]);
    const positions=new Map<string,{x:number;y:number}>();
    for(const op of ops){
      const level=Math.max(0,...(incoming.get(op.id)??[]).map(id=>(levels.get(id)??-1)+1));
      levels.set(op.id,level);const row=rows.get(level)??0; rows.set(level,row+1);
      positions.set(op.id,{x:level*270,y:row*112});
    }
    if(focusId){
  ...
  const fit=()=>{const bounds=canvas.current?.getBoundingClientRect();if(!bounds||!positions.size)return;const width=Math.max(...[...positions.values()].map(p=>p.x))+218;const height=Math.max(...[...positions.values()].map(p=>p.y))+76;const k=Math.min(1.5,(bounds.width-70)/width,(bounds.height-110)/height);setTransform({x:35,y:35,k:Math.max(.8,k)})};
  const zoom=(factor:number)=>setTransform(t=>({...t,k:Math.max(.12,Math.min(3,t.k*factor))}));
  return <>{boundary(inputIds,"Inputs to this graph slice")}<div className={`graph-wrap ${focusId?'local-graph':''}`}>
    ...
    <svg ref={canvas} className="graph" style={focusId?{height:Math.max(400,...[...positions.values()].map(p=>p.y+170)),minWidth:850}:undefined} aria-label="Computation graph" onWheel={e=>{zoom(e.deltaY<0?1.08:1/1.08)}} onPointerDown={e=>{if(e.target===e.currentTarget){drag.current={x:e.clientX,y:e.clientY}; e.currentTarget.setPointerCapture(e.pointerId)}}} onPointerMove={e=>{if(drag.current){const dx=e.clientX-drag.current.x,dy=e.clientY-drag.current.y;setTransform(t=>({...t,x:t.x+dx,y:t.y+dy}));drag.current={x:e.clientX,y:e.clientY}}}} onPointerUp={()=>{drag.current=null}}>
      <defs><marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8" fill="#71839b"/></marker></defs>
      <g transform={`translate(${transform.x},${transform.y}) scale(${transform.k})`}>
        {edges.map((e,i)=>{const a=positions.get(e.source_id)!,b=positions.get(e.target_id)!;const d=`M${a.x+218},${a.y+36} C${a.x+248},${a.y+36} ${b.x-30},${b.y+36} ${b.x},${b.y+36}`;return <g key={i} className="graph-edge" onClick={()=>select({kind:'tensor',id:e.tensor_id})}><path d={d} fill="none" stroke="transparent" strokeWidth="14"/><path d={d} fill="none" stroke={selected.id===e.tensor_id?'#087f8c':'#8f9db0'} strokeWidth="1.5" markerEnd="url(#arrow)"/><title>{e.tensor_id} {shape(tensors.get(e.tensor_id))}</title>{edges.length<80&&<text x={(a.x+218+b.x)/2} y={(a.y+b.y)/2+27} textAnchor="middle" className="edge-shape">{shape(tensors.get(e.tensor_id)).slice(0,24)}</text>}</g>})}
        {ops.map(op=>{const p=positions.get(op.id)!;return <g key={op.id} tabIndex={0} role="button" aria-label={op.display_name} onPointerDown={e=>{e.stopPropagation();nodeMoved.current=false;nodeDrag.current={id:op.id,x:e.clientX,y:e.clientY};setDraggingId(op.id);e.currentTarget.setPointerCapture(e.pointerId)}} onPointerMove={e=>{if(nodeDrag.current?.id!==op.id)return;const dx=(e.clientX-nodeDrag.current.x)/transform.k,dy=(e.clientY-nodeDrag.current.y)/transform.k;if(dx||dy)nodeMoved.current=true;setOffsets(previous=>({...previous,[op.id]:{x:(previous[op.id]?.x??0)+dx,y:(previous[op.id]?.y??0)+dy}}));nodeDrag.current={id:op.id,x:e.clientX,y:e.clientY}}} onPointerUp={e=>{if(nodeDrag.current?.id===op.id){nodeDrag.current=null;setDraggingId(null);e.currentTarget.releasePointerCapture(e.pointerId)}}} onPointerCancel={e=>{if(nodeDrag.current?.id===op.id){nodeDrag.current=null;setDraggingId(null);e.currentTarget.releasePointerCapture(e.pointerId)}}} onKeyDown={e=>{if(e.key==='Enter')select({kind:'operation',id:op.id})}} onClick={()=>{if(nodeMoved.current){nodeMoved.current=false;return}select({kind:'operation',id:op.id})}} transform={`translate(${p.x},${p.y})`} className={`op-node ${draggingId===op.id?'dragging':''}`}>
...
export function GraphView({ir,ops,select,selected,inspection=emptyInspection}:{ir:IR;ops:Operation[];select:(s:Selection)=>void;selected:Selection;inspection?:Inspection}){
 ...
 const current=ops.find(o=>o.id===focus)??ops[0];
 ...
 const shown=raw?ops:current?[current,...neighbors.slice(0,6)]:[];
 return <div className="compute-experience"><div className="focus-controls"><div><span className="eyebrow">{raw?'FULL SCOPE':'ONE OPERATION AT A TIME'}</span><h3>{raw?'Raw computation graph':'Follow the tensor flow'}</h3></div><button aria-pressed={raw} onClick={()=>setRaw(r=>!r)}>{raw?'Focused view':'Raw graph'}</button></div>{current&&<><div className="operation-navigation"><button disabled={index===0} onClick={()=>choose(index-1)}>Previous operation</button><label>Operation <select aria-label="Focused operation" value={current.id} onChange={e=>choose(ops.findIndex(o=>o.id===e.target.value))}>{ops.map((o,i)=><option key={o.id} value={o.id}>{i+1}. {o.display_name}</option>)}</select></label><button disabled={index===ops.length-1} onClick={()=>choose(index+1)}>Next operation</button><span>{index+1} / {ops.length}</span></div><OperationExplainer ir={ir} op={current} select={select} inspection={inspection}/></>}{!raw&&<p className="neighborhood-note">Direct inputs ← selected operation → direct consumers. Only recorded edges are drawn.{neighbors.length>6?` Showing 6 of ${neighbors.length} adjacent operations; use the selector or Raw graph for the rest.`:''} Navigation follows trace listing order; it does not imply a dependency.</p>}<GraphCanvas key={`${raw}-${current?.id}`} ir={ir} ops={shown} select={select} selected={selected.kind==='tensor'?selected:{kind:'operation',id:current?.id??''}} focusId={raw?undefined:current?.id}/></div>;
}

...
4. In **Edit**, enable bounded CPU samples and **Build & Re-trace**. Return to an early operation to inspect its captured prefix. Change a slice index to demonstrate that uncaptured values are explicitly unavailable.
5. At root, select `norm`. In **Edit**, change `eps` to `0.001`, **Preview & validate**, **Commit edit**, then **Build & Re-trace**.
6. Open **Compare**. Show observed r0/r1, actual constructor changes, shape/sample changes and the aligned operation microscopes. A safe epsilon edit need not change operation counts or shapes; unchanged evidence is informative.

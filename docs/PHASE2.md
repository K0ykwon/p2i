...
Optional bounded CPU samples can be enabled for Build & Re-trace. They are stored in a separate in-memory sidecar, not saved ModelIR. Defaults: at most 64 tensors, 4,096 elements per tensor for sampling/statistics, 16 displayed values per tensor, and 256 operations' compact scalar arguments. Hard caps prevent increasing these without limit. Large/device/layout-incompatible tensors are omitted with a reason. Inspection arithmetic is excluded from the model operation trace. Without opt-in, the original metadata-only behavior remains.
